export function footer() {
  ...
  footerInner.style.transform = 'translateX(-50%) translateY(100%)';
  ...
  window.addEventListener('scroll', () => {
    ...
    if (scrollY + windowHeight > footerOffset) {
      footerInner.style.transform = 'translateX(-50%) translateY(0%)'; // 푸터가 화면에 보이도록
    } else {
      footerInner.style.transform = 'translateX(-50%) translateY(100%)'; // 화면 아래로 내려가도록
    }
  });
}
